...
{
	Matrix3x3 tMat = GetTMatrix();
	Matrix3x3 rMat = GetRMatrix();
	Matrix3x3 sMat = Matrix3x3(Vector3::UnitX * Scale.X, Vector3::UnitY * Scale.Y, Vector3::UnitZ);

	return tMat * rMat * sMat;
}

Matrix3x3 Transform2D::GetTMatrix() const
{
	return Matrix3x3(Vector3::UnitX, Vector3::UnitY, Vector3(Position.X, Position.Y, 1.f));
}

Matrix3x3 Transform2D::GetRMatrix() const
{
	float rad = Math::Deg2Rad(RotationInDegree);
	return Matrix3x3(Vector3(cosf(rad), sinf(rad), 0.f), Vector3(-sinf(rad), cosf(rad), 0.f), Vector3::UnitZ);
}
